...
const core: EmscriptenApi = await perspective_server.default({ wasmBinary });
// await (core as any).__initialize(); // this is generated by wasm-bindgen?
...
// await perspective_client.__initialize();
    ...
    constructor({ port = 8080, assets = ["./"], hostPsp = false } = {}) {
        ...
        this._wss.on("connection", (ws) => {
            ...
            const server = make_server((proto: Uint8Array) => {
                console.log("Sending proto: ", proto);
                ws.send(buffer_to_arraybuffer(proto));
            });

            ws.on("message", (proto) => {
                console.log("Got response!!! ", proto);
                const responses = server.handle_message(
                    buffer_to_arraybuffer(proto)
                );
                for (const resp of responses) {
                    CLIENTS.get(resp.client_id)!(resp.data);
                }

                setTimeout(() => {
                    const polled = server.poll();
                    for (const resp of polled) {
                        CLIENTS.get(resp.client_id)!(resp.data);
                    }
                });
            });
        ...
        this._server.listen(port, () => {
            console.log(`Listening on ${this._server.address()}`);
        });
    }
...
SYNC_SERVER = make_server((resp) => {
    console.log("handling message... 1?", resp);
    SYNC_CLIENT.handle_message(resp);
});

SYNC_CLIENT = make_client((req) => {
    return SYNC_SERVER.handle_message(req);
});
