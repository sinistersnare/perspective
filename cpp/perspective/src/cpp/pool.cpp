...
t_updctx::t_updctx(t_uindex gnode_id, std::string ctx)
    : m_gnode_id(gnode_id), m_ctx(std::move(ctx)) {}
...
t_uindex t_pool::register_gnode(t_gnode *node) {
...
void t_pool::unregister_gnode(t_uindex idx) {
...
void t_pool::send(t_uindex gnode_id, t_uindex port_id,
                  const t_data_table &table) {
  {
    ...
    if (t_env::log_progress()) {
      std::cout << "t_pool.send gnode_id => " << gnode_id << " port_id => "
                << port_id << " tbl_size => " << table.size() << std::endl;
    }
...
#ifdef PSP_PARALLEL_FOR
std::shared_mutex *t_pool::get_lock() const { return m_lock; }
#endif

void t_pool::_process(
...
void t_pool::set_sleep(t_uindex ms) {
  m_sleep.store(ms);
  if (t_env::log_progress()) {
...
std::vector<t_stree *> t_pool::get_trees() {
...
void t_pool::register_context(t_uindex gnode_id, const std::string &name,
...
void t_pool::notify_userspace(t_uindex port_id) {
  // #if defined PSP_ENABLE_WASM && !defined PSP_ENABLE_PYTHON
...
void t_pool::unregister_context(t_uindex gnode_id, const std::string &name) {
  std::lock_guard<std::mutex> lg(m_mtx);
...
bool t_pool::get_data_remaining() const {
  auto data = m_data_remaining.load();
  return data;
}

std::vector<t_tscalar>
t_pool::get_row_data_pkeys(t_uindex gnode_id,
...
std::vector<t_updctx> t_pool::get_contexts_last_updated() {
  std::lock_guard<std::mutex> lg(m_mtx);
...
bool t_pool::validate_gnode_id(t_uindex gnode_id) const {
  return (m_gnodes[gnode_id] != nullptr) && gnode_id < m_gnodes.size();
}

std::string t_pool::repr() const {
  std::stringstream ss;
  ss << "t_pool<" << this << ">";
  return ss.str();
}

void t_pool::pprint_registered() const {
  auto self = repr();
...
t_uindex t_pool::epoch() const { return m_epoch.load(); }

void t_pool::inc_epoch() { ++m_epoch; }

std::vector<t_uindex> t_pool::get_gnodes_last_updated() {
  std::lock_guard<std::mutex> lg(m_mtx);
...
t_gnode *t_pool::get_gnode(t_uindex idx) {
  std::lock_guard<std::mutex> lg(m_mtx);
