...
static std::string re_bool_to_str(std::string &&expression) {
...
static std::string replace_with_re2(const std::string &input,
                                    const re2::RE2 &pattern, Lambda replacer) {
  std::string output;
  re2::StringPiece input_piece(input);

  PSP_VERBOSE_ASSERT1(pattern.NumberOfCapturingGroups() >= 2,
...
static auto re_column_name_to_id(std::string &&expression,
  ...
  auto parsed_expression_string =
      replace_with_re2(expression, column_name,
...
static auto re_intern_strings(std::string &&expression) {
  static const RE2 intern_string("('.*?[^\\\\]')");
  RE2::GlobalReplace(&expression, intern_string, "intern(\\1)");
  return expression;
}

static auto re_unintern_some_exprs(std::string &&expression) {
...
parse_expression_strings(const F &column_expr) {
  ...
  for (const auto &[colname, expr] : column_expr) {
    ...
    std::tie(validated_expr.parse_expression_string,
             validated_expr.column_id_map) =
        re_column_name_to_id(std::move(validated_expr.parse_expression_string),
                             validated_expr);
...
void ServerResources::host_table(const t_id &id, std::shared_ptr<Table> table) {
  m_tables.emplace(id, std::move(table));
}

void ServerResources::host_view(const t_id &id, const t_id &table_id,
...
std::shared_ptr<Table> ServerResources::get_table(const t_id &id) {
  return m_tables.at(id);
}

std::vector<ServerResources::t_id> ServerResources::get_table_ids() {
  std::vector<t_id> vec;
...
std::shared_ptr<ErasedView> ServerResources::get_view(const t_id &id) {
  return m_views.at(id);
}

void ServerResources::delete_view(const t_id &id) {
...
void ServerResources::delete_table(const t_id &id) { m_tables.erase(id); }

void ServerResources::mark_table_dirty(const t_id &id) {
  m_dirty_tables.insert(id);
}

void ServerResources::mark_table_clean(const t_id &id) {
  m_dirty_tables.erase(id);
}

void ServerResources::mark_all_tables_clean() { m_dirty_tables.clear(); }

void ServerResources::create_table_on_delete_sub(const t_id &table_id,
                                                 Subscription sub_id) {
...
void ServerResources::drop_table_on_delete_sub(const t_id &table_id,
...
void ServerResources::create_view_on_delete_sub(const t_id &view_id,
                                                Subscription sub) {
...
void ServerResources::drop_view_on_delete_sub(const t_id &view_id,
                                              const std::uint32_t sub_id,
                                              const std::uint32_t client_id) {
  if (!m_view_on_delete_subs.contains(view_id)) {
    m_view_on_delete_subs[view_id] = {};
...
void ServerResources::create_view_on_update_sub(const t_id &view_id,
                                                Subscription sub) {
...
void ServerResources::drop_view_on_update_sub(const t_id &view_id) {
...
bool ServerResources::is_table_dirty(const t_id &id) {
  return m_dirty_tables.contains(id);
}

std::vector<ProtoServerResp<std::string>>
ProtoServer::handle_message(std::uint32_t client_id,
...
std::vector<ProtoServerResp<std::string>> ProtoServer::poll() {
...
proto::ColumnType dtype_to_column_type(const t_dtype &t) {
  switch (t) {
...
t_dtype column_type_to_dtype(const proto::ColumnType &t) {
  switch (t) {
...
static ValidViewPort parse_format_options(
    const proto::ViewPort &viewport, std::uint32_t num_columns,
    std::uint32_t num_rows, std::uint32_t sides, bool column_only,
    std::uint32_t num_hidden, std::uint32_t viewport_top = 0,
    std::uint32_t viewport_left = 0, std::uint32_t viewport_height = 0,
  ...
  out.end_row = std::min(max_rows, viewport.has_end_row()
                                       ? viewport.end_row()
                                       : (viewport_height != 0
                                              ? out.start_row + viewport_height
                                              : max_rows));
  out.end_col = std::min(
      max_cols, (viewport.has_end_col()
                     ? viewport.end_col() + psp_offset
                     : (viewport_width != 0 ? out.start_col + viewport_width
                                            : max_cols)) *
                    (hidden + 1));
...
void ProtoServer::handle_process_table(
    const Request &req,
...
static std::string_view view_sides_to_string(const ErasedView &view) {
  switch (view.sides()) {
  case 0:
    return "zero";
  case 1:
...
static std::uint32_t calculate_num_hidden(const ErasedView &view,
  ...
  case 2: {
    ...
    for (const auto &s : config.get_sortspec()) {
      ...
      if (std::find(cols.begin(), cols.end(), s.m_colname) == cols.end()) {
...
template <typename A>
static t_tscalar coerce_to(const t_dtype dtype, const A &val) {
  if constexpr (std::is_same_v<A, std::string>) {
    ...
    case DTYPE_TIME: {
      ...
      t_time time{std::chrono::duration_cast<std::chrono::milliseconds>(
                      tp.time_since_epoch())
                      .count()};
    ...
    case DTYPE_DATE: {
      ...
      t_date date{static_cast<std::int16_t>(tm.tm_year + 1900),
    ...
    default:
      PSP_COMPLAIN_AND_ABORT("Column of type " + dtype_to_str(dtype) +
...
ProtoServer::_handle_message(std::uint32_t client_id, const Request &req) {
  ...
  case proto::Request::kGetFeaturesReq: {
    ...
    (*features->mutable_filter_ops())[proto::ColumnType::INTEGER] = opts2;
    (*features->mutable_filter_ops())[proto::ColumnType::FLOAT] = opts2;
    (*features->mutable_filter_ops())[proto::ColumnType::DATE] = opts2;
    (*features->mutable_filter_ops())[proto::ColumnType::DATETIME] = opts2;
    ...
    opts.add_options("is not null");
    opts.add_options("is null");
    (*features->mutable_filter_ops())[proto::ColumnType::BOOLEAN] = opts2;
  ...
  case proto::Request::kMakeTableReq: {
    ...
    case proto::MakeTableReq_MakeTableOptions::MAKE_TABLE_TYPE_NOT_SET:
      break;
    }
    ...
    case proto::MakeTableData::kFromView: {
      ...
      auto dims = parse_format_options(
          viewport, view->num_columns(), view->num_rows(), view->sides(),
          view->get_view_config()->is_column_only(), 0);
      auto arrow = view->to_arrow(dims.start_row, dims.end_row, dims.start_col,
                                  dims.end_col);
    ...
    case proto::MakeTableData::kFromArrow: {
      table = Table::from_arrow(index, r.data().from_arrow(), limit);
      break;
    }
    ...
    case proto::MakeTableData::kFromCols: {
      table = Table::from_cols(index, r.data().from_cols(), limit);
      break;
    }
    case proto::MakeTableData::kFromRows: {
      table = Table::from_rows(index, r.data().from_rows(), limit);
      break;
    }
  ...
  case proto::Request::kTableSchemaReq: {
    ...
    proto::Response resp;
    auto *output_schema = resp.mutable_table_schema_resp()->mutable_schema();
  ...
  case proto::Request::kTableValidateExprReq: {
    ...
    for (const auto &expr : exprs) {
      legacy_exprs.emplace_back(
          expr.expression_alias, expr.expression, expr.parse_expression_string,
          std::vector<std::pair<std::string, std::string>>{
    ...
    for (const auto &[col, val] : res.get_expression_schema()) {
      schema.emplace_back(col, dtype_to_column_type(str_to_dtype(val)));
    }

    for (auto &&[col_name, col_type] : schema) {
      (*validate_expr->mutable_expression_schema())[col_name] = col_type;
    }
    ...
    for (const auto &[col_name, err] : res.get_expression_errors()) {
      ...
      *proto_err.mutable_error_message() = err.m_error_message;
      ...
      (*validate_expr->mutable_errors())[col_name] = std::move(proto_err);
    }

    for (const auto &[col_name, col_expr] : r.column_to_expr()) {
      (*validate_expr->mutable_expression_alias())[col_name] = col_expr;
    }
  ...
  case proto::Request::kTableMakeViewReq: {
    ...
    std::vector<std::string> row_pivots{group_by.begin(), group_by.end()};

    const auto &split_by = cfg.split_by();
    std::vector<std::string> column_pivots{split_by.begin(), split_by.end()};
    ...
    for (const auto &expr : exprs) {
      ...
      auto column_id_map = std::vector<std::pair<std::string, std::string>>(
          expr.column_id_map.begin(), expr.column_id_map.end());

      auto expr_vocab = gnode->get_expression_vocab();
      t_expression_vocab &expression_vocab = *expr_vocab;
      auto expression_regex_mapping = gnode->get_expression_regex_mapping();
      ...
      std::shared_ptr<t_computed_expression> computed_expression =
          t_computed_expression_parser::precompute(
              expr.expression_alias, expr.expression,
              expr.parse_expression_string, column_id_map,
      ...
      for (const auto &arg : f.value()) {
        ...
        case proto::Scalar::kString: {
          if (!schema->has_column(f.column())) {
            PSP_COMPLAIN_AND_ABORT("Filter column not in schema: " +
                                   f.column());
          }
          a = coerce_to(schema->get_dtype(f.column()), arg.string());
          args.push_back(a);
          break;
        }
        ...
        case proto::Scalar::kDate: {
          ...
          t_date d{static_cast<std::int16_t>(date->tm_year + 1900),
        ...
        case proto::Scalar::SCALAR_NOT_SET:
          PSP_COMPLAIN_AND_ABORT("Filter scalar type not implemented: " +
    ...
    std::vector<std::string> columns;
    if (cols.has_columns()) {
      columns = {cols.columns().columns().begin(),
    ...
    LOG_DEBUG("Creating view config with \n"
    ...
    case proto::ViewConfig_FilterReducer::ViewConfig_FilterReducer_OR:
      filter_op = "or";
      break;
    case proto::ViewConfig_FilterReducer::ViewConfig_FilterReducer_AND:
    default:
      filter_op = "and";
      break;
    }
    ...
    auto config = std::make_shared<t_view_config>(
        row_pivots, column_pivots, aggregates, columns, filter, sort_str,
        expressions, filter_op, column_only);
    config->init(schema);
    ...
    bool is_unit_context = table->get_index().empty() && sides == 0 &&
                           ...
                           aggregates.empty() && columns.empty() &&
                           sort_str.empty() && cfg.expressions().empty();

    std::shared_ptr<ErasedView> erased_view;

    if (is_unit_context) {
      auto ctx = make_context<t_ctxunit>(table, schema, config, r.view_id());
      auto view = std::make_shared<View<t_ctxunit>>(table, ctx, r.view_id(),
                                                    "|", config);
      erased_view = std::make_shared<CtxUnitView>(std::move(view));
    } else if (sides == 0) {
      auto ctx = make_context<t_ctx0>(table, schema, config, r.view_id());
      auto view =
          std::make_shared<View<t_ctx0>>(table, ctx, r.view_id(), "|", config);
      erased_view = std::make_shared<Ctx0View>(view);
    } else if (sides == 1) {
      auto ctx = make_context<t_ctx1>(table, schema, config, r.view_id());
      auto view =
          std::make_shared<View<t_ctx1>>(table, ctx, r.view_id(), "|", config);
      erased_view = std::make_shared<Ctx1View>(std::move(view));
    } else if (sides == 2) {
      auto ctx = make_context<t_ctx2>(table, schema, config, r.view_id());
      auto view =
          std::make_shared<View<t_ctx2>>(table, ctx, r.view_id(), "|", config);
  ...
  case proto::Request::kTableRemoveDeleteReq: {
    ...
    m_resources.drop_table_on_delete_sub(req.entity_id(), sub_id, client_id);
    proto::Response resp;
  ...
  case proto::Request::kViewRemoveDeleteReq: {
    ...
    m_resources.drop_view_on_delete_sub(req.entity_id(), sub_id, client_id);
    proto::Response resp;
  ...
  case proto::Request::kViewSchemaReq: {
    ...
    proto::Response resp;
    auto *view_schema = resp.mutable_view_schema_resp()->mutable_schema();
  ...
  case proto::Request::kViewDimensionsReq: {
    ...
    auto num_view_columns =
        ncols -
        (ncols / (config->get_columns().size() + num_hidden)) * num_hidden;
  ...
  case proto::Request::kViewGetConfigReq: {
    ...
    for (const auto &col : view_config->get_columns()) {
      view_config_proto->mutable_columns()->mutable_columns()->add_columns(col);
    }
    ...
    for (const auto &filter : view_config->get_fterm()) {
      ...
      if (filter.m_op != FILTER_OP_NOT_IN && filter.m_op != FILTER_OP_IN) {
      ...
      for (const auto &scalar : vals) {
        ...
        case DTYPE_FLOAT64:
          ...
          break;
        case DTYPE_INT64:
          s->set_int_(static_cast<std::int32_t>(scalar.get<std::int64_t>()));
          break;
        case DTYPE_STR:
        ...
        case DTYPE_TIME:
          ...
          break;
        case DTYPE_NONE:
          s->set_null(::google::protobuf::NullValue::NULL_VALUE);
          break;
        default:
          PSP_COMPLAIN_AND_ABORT("Invalid scalar type: " + scalar.to_string());
        }
      }
    }
    ...
    case FILTER_OP_OR:
      view_config_proto->set_filter_op(
          proto::ViewConfig_FilterReducer::ViewConfig_FilterReducer_OR);
      break;
    case FILTER_OP_AND:
    default:
      view_config_proto->set_filter_op(
          proto::ViewConfig_FilterReducer::ViewConfig_FilterReducer_AND);
      break;
    }

    if (view_config->get_row_pivot_depth() != -1) {
      view_config_proto->set_group_by_depth(view_config->get_row_pivot_depth());
    }
  ...
  case proto::Request::kViewToRowsStringReq: {
    ...
    auto dims = parse_format_options(r.viewport(), view->num_columns(),
                                     view->num_rows(), view->sides(),
                                     config->is_column_only(), num_hidden);

    auto json_str = view->to_rows(
        dims.start_row, dims.end_row, dims.start_col, dims.end_col, num_hidden,
        r.formatted(), r.index(), r.id(), r.leaves_only(), view->sides(),
        view->sides() > 0 && !config->is_column_only(), nidx,
  ...
  case proto::Request::kViewToColumnsStringReq: {
    ...
    auto json_str = view->to_columns(
        ...
        r.formatted(), r.index(), r.id(), r.leaves_only(), view->sides(),
        view->sides() > 0 && !config->is_column_only(), nidx,
        config->get_columns().size(), config->get_row_pivots().size());
  ...
  case proto::Request::kViewRemoveOnUpdateReq: {
    ...
    m_resources.remove_update_subscription(req.entity_id(), sub_id, client_id);
  ...
  case proto::Request::kViewToArrowReq: {
    ...
    auto num_hidden = calculate_num_hidden(*view, *config);
    auto dims = parse_format_options(
        r.viewport(), view->num_columns(), view->num_rows(), view->sides(),
        view->get_view_config()->is_column_only(), num_hidden);

    proto::Response resp;
    auto *arrow = resp.mutable_view_to_arrow_resp()->mutable_arrow();
    *arrow = *view->to_arrow(dims.start_row, dims.end_row, dims.start_col,
                             dims.end_col, true, r.compression() == "lz4");
  ...
  case proto::Request::kViewToCsvReq: {
    ...
    *csv = *view->to_csv(dims.start_row, dims.end_row, dims.start_col,
                         dims.end_col);
...
std::vector<ProtoServerResp<ProtoServer::Response>> ProtoServer::_poll() {
...
void ProtoServer::_process_table_unchecked(
    std::shared_ptr<Table> &table, const ServerResources::t_id &table_id,
...
void ProtoServer::_process_table(
    std::shared_ptr<Table> &table, const ServerResources::t_id &table_id,
    std::vector<ProtoServerResp<ProtoServer::Response>> &outs) {
  if (!m_resources.is_table_dirty(table_id)) {
    return;
...
void ServerResources::remove_update_subscription(const t_id &view_id,
                                                 std::uint32_t sub_id,
...
const char *perspective::sort_op_str_from_proto(proto::SortOp sort_op) {
  switch (sort_op) {
