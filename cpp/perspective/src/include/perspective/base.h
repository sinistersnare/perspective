 ...
 */
[[noreturn]] PERSPECTIVE_EXPORT void psp_abort(const std::string &message);
...
#define PSP_COMPLAIN_AND_ABORT(X)                                              \
  { psp_abort("TODO: Davis fill this in"); }
#else
#define PSP_COMPLAIN_AND_ABORT(X)                                              \
  {                                                                            \
...
#define PSP_VERBOSE_ASSERT(...)                                                \
  _ID(GET_PSP_VERBOSE_ASSERT(__VA_ARGS__, PSP_VERBOSE_ASSERT2,                 \
                             PSP_VERBOSE_ASSERT1)(__VA_ARGS__))
...
template <typename T>
inline std::ostream &operator<<(std::ostream &os, const std::vector<T> &row) {
...
inline std::ostream &operator<<(std::ostream &os,
                                const std::pair<FIRST_T, SECOND_T> &p) {
  os << "<" << p.first << ", " << p.second << ">";
  return os;
}
...
struct t_cchar_umap_hash {
  inline t_uindex operator()(const char *s) const {
...
template <typename DATA_T> std::string psp_to_str(const DATA_T &s) {
  std::stringstream ss;
  ss << s;
  return ss.str();
}

template <typename T> PERSPECTIVE_EXPORT t_dtype type_to_dtype();

template <> PERSPECTIVE_EXPORT t_dtype type_to_dtype<std::int64_t>();

template <> PERSPECTIVE_EXPORT t_dtype type_to_dtype<std::int32_t>();

template <> PERSPECTIVE_EXPORT t_dtype type_to_dtype<std::int16_t>();

template <> PERSPECTIVE_EXPORT t_dtype type_to_dtype<std::int8_t>();

template <> PERSPECTIVE_EXPORT t_dtype type_to_dtype<std::uint64_t>();

template <> PERSPECTIVE_EXPORT t_dtype type_to_dtype<std::uint32_t>();

template <> PERSPECTIVE_EXPORT t_dtype type_to_dtype<std::uint16_t>();

template <> PERSPECTIVE_EXPORT t_dtype type_to_dtype<std::uint8_t>();

template <> PERSPECTIVE_EXPORT t_dtype type_to_dtype<double>();

template <> PERSPECTIVE_EXPORT t_dtype type_to_dtype<float>();

template <> PERSPECTIVE_EXPORT t_dtype type_to_dtype<bool>();

template <> PERSPECTIVE_EXPORT t_dtype type_to_dtype<t_time>();

template <> PERSPECTIVE_EXPORT t_dtype type_to_dtype<t_date>();

template <> PERSPECTIVE_EXPORT t_dtype type_to_dtype<std::string>();
